...
<div class="max-w-7xl w-full">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100">Form Edit Berita</h3>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Edit informasi berita di bawah ini</p>
            </div>
            <a 
        ...
        <form action="{{ route('admin.news.update', $news->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select 
                                ...
                                class="w-full px-4 py-3 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                            >
                                <option value="Featured" {{ old('category', $news->category) == 'Featured' ? 'selected' : '' }}>Featured</option>
                                <option value="Hot" {{ old('category', $news->category) == 'Hot' ? 'selected' : '' }}>Hot</option>
                                <option value="New" {{ old('category', $news->category) == 'New' ? 'selected' : '' }}>New</option>
                                ...
                                <option value="Info" {{ old('category', $news->category) == 'Info' ? 'selected' : '' }}>Info</option>
                                <option value="Trending" {{ old('category', $news->category) == 'Trending' ? 'selected' : '' }}>Trending</option>
                            </select>
                            @error('category')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
            ...
            <div class="border-b border-gray-200 dark:border-gray-700 pb-6">
                ...
                @if($news->image)
                    <div class="mb-4">
                        ...
                        <img 
                            ...
                            alt="{{ $news->title }}"
                            class="w-48 h-32 object-cover rounded-lg border border-gray-300 dark:border-gray-700"
                        >
                    </div>
                @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {{ $news->image ? 'Ganti Gambar Featured' : 'Gambar Featured' }}
                        </label>
                        <input 
                            type="file" 
                            name="image" 
                ...
                <div class="mt-4 space-y-3">
                    <label class="flex items-center gap-3 cursor-pointer">
                        ...
                        >
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Publikasikan</span>
                    </label>
            ...
            <div class="flex items-center justify-end gap-4">
                ...
                >
                    <i class="ri-save-line"></i> <span id="submit-text">Update Berita</span>
                </button>
            </div>
        </form>
    </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Set initial content
        @if(old('content', $news->content))
            var initialContent = {!! json_encode(old('content', $news->content)) !!};
            if (initialContent) {
